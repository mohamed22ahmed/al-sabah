...
export default {
    ...
    watch: {
        banner: {
            handler(newBanner) {
                if (newBanner && Object.keys(newBanner).length > 0) {
                    this.populateForm();
                }
            },
            immediate: true,
    ...
    methods: {
        populateForm() {
            this.form = {
                show: this.banner.show || false,
                image: null,
            };
            this.imagePreview = this.banner.image;
        },
        ...
        async submit() {
            ...
            try {
                const response = await axios.post(`/admin/banners/update/${this.banner.id}`, formData, {
                    headers: {
                        'Content-Type': 'multipart/form-data',
                    },
                });

                this.$emit('updated', this.banner);
            } catch (error) {
    ...
    <Modal :show="show" @close="$emit('close')">
        <div class="p-6" style="direction: rtl;">
            <div class="flex justify-between items-center mb-4">
                <h5 class="text-lg font-bold">تعديل البانر</h5>
