    ...
    methods: {
        async fetchCategoryProducts() {
            try {
                this.loading = true;
                this.error = null;
                const response = await fetch('/api/categories/'+this.category.id+'/products');
                ...
                if (data) {
                    ...
                    this.error = data.message || 'حدث خطأ في تحميل المنتجات';
                }
            } catch (error) {
...
<template>
    <section class="py-16 bg-white" style="direction: rtl;">
        <div class="max-w-6xl mx-auto px-4">
            ...
            <!-- View All Link -->
            <div v-if="products.length > 0" class="text-left mb-4">
                <a
                    :href="category.url"
